...
let TCGs = data.TCG;
TCGs = TCGs.map((TCG) => TCG.name + ".png");
...
$: {
  if (searchTerm) {
    filteredTCGs = TCGs.filter((TCG) => {
      return TCG.toLowerCase().includes(
...
// function to download all TCG
async function downloadAll() {
  console.log("downloading all TCG");
  downloadToast.showToast();

  const zip = new JSZip();
  const folder = zip.folder("TCG");
  for (let i = 0; i < TCGs.length; i++) {
    const response = await fetch(`/images/TCG/${TCGs[i]}`);
    const blob = await response.blob();
    folder.file(TCGs[i], blob);
  }
  try {
    zip.generateAsync({ type: "blob" }).then(function (content) {
      // try to save the zip file, if it doesn't work, show an error message
      try {
        saveAs(content, "TCG.zip");
        setTimeout(() => {
...
// download selected TCG
async function downloadSelected() {
  console.log("downloading selected TCG");
  downloadToast.showToast();

  const zip = new JSZip();
  const folder = zip.folder("TCG");
  for (let i = 0; i < selectedItems.length; i++) {
    const response = await fetch(`/images/TCG/${selectedItems[i]}`);
    const blob = await response.blob();
...
<svelte:head>
  <title>genius invokation tcg assets | wanderer.moe</title>
</svelte:head>
    ...
    <div class="container mx-auto px-4">
      <div class="flex flex-wrap justify-between gap-4">
        <h1 class="font-semibold text-white">
          ...
          {#if selectedItems.length > 0}
            <span class="text-xs font-normal text-gray-400"
              >You can also download <button
                ...
                class="font-semibold text-white hover:text-indigo-400"
                >selected TCG assets (.zip) here</button
              >.</span
            >
          ...
          {:else}
            <span class="text-xs font-normal text-gray-400"
              >You can also select multiple assets and download as .zip</span
            >
          {/if}
        </h1>
        ...
        {#each filteredTCGs as entry}
          ...
          >
            ...
            <div>
              ...
              <a
                href="/images/tcg/{entry}"
                target="_blank"
                rel="noreferrer"
                download
              >
                <button
                  class="justify-right rounded-lg bg-indigo-400 bg-opacity-70 px-5 py-2.5 font-semibold text-white hover:bg-indigo-500 focus:shadow focus:outline-none"
                >
