...
async function downloadAll() {
  ...
  for (let i = 0; i < assets.length; i++) {
    try {
      const response = await fetch(`/images/tcg/${assets[i]}`);
...
async function downloadSelected() {
  ...
  const zip = new JSZip();
  const folder = zip.folder("tcg-selected");
  ...
  try {
    zip.generateAsync({ type: "blob" }).then(function (content) {
      // try to save the zip file, if it doesn't work, show an error message
      try {
        saveAs(content, "tcg-selected.zip");
        setTimeout(() => {
          downloadToast.hideToast();
        }, 5000);
      } catch (error) {
...
<div class="flex min-h-screen flex-col">
  <section class="py-24">
    <div class="container mx-auto bg-[#121212] px-4">
      <div class="mb-6">
        <h1 class="gifont font-semibold text-white">Genius Invokation TCG</h1>
        <p class="text-sm text-gray-400">
          Genius Invokation TCG assets, including full cards, character parts
          and other TCG assets.
        </p>
        <p class="text-sm text-gray-400">
